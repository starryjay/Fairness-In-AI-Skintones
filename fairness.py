...
def get_agreement(gtpath: str, morphepath: str, clusterpath: str) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    gt_morphe.loc[:, 'Skin Tone Category'] = gt_morphe.apply(lambda x: skintone_map[stats.mode(set([x['ground_truth1'].split('_')[0],
    ...
    gt_cluster.loc[:, 'Skin Tone Category'] = gt_cluster.apply(lambda x: skintone_map[stats.mode(set([x['ground_truth1'].split('_')[0], 
                                                                                       x['ground_truth2'].split('_')[0],
                                                                                       x['ground_truth3'].split('_')[0],
                                                                                       x['ground_truth4'].split('_')[0],
                                                                                       x['ground_truth5'].split('_')[0]]))], axis=1)
...
def fairness(grouped_morphe: pd.DataFrame, grouped_cluster: pd.DataFrame) -> tuple[float, float, float]:
